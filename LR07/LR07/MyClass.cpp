...
MyClass::MyClass() {
	...
	hEventBufEmpty = CreateEvent(NULL, FALSE, TRUE, NULL); //в начале буфер пуст 
	hEventBufFull = CreateEvent(NULL, FALSE, FALSE, NULL); //буфер заполнен
...
{
	WaitForSingleObject(hEventBufEmpty, INFINITE);//ждем пока буфер не заполнится
	buf_key = key;
	buf_mark = mark;
	SetEvent(hEventBufFull);//сигнальное состояние, что буфер заполнен
}
