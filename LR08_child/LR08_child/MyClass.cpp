...
MyClass::MyClass() {
	...
	hEventBufEmpty = CreateEvent(NULL, FALSE, FALSE, L"BuffEmpty.txt");//изначально буфер чист
	hEventBufFull = CreateEvent(NULL, FALSE, TRUE, L"BuffFull.txt");//событие занято
}
...
void MyClass::add_to_list(int key, int mark) //добавление пары ключ-значение в список
{
	...
	{
		myList.push_back(make_tuple(key, mark));
